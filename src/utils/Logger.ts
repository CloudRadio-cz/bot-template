...
export class Logger {
	...
	private static formatMessage(level: string, message: string): string {
		const timestamp = chalk.gray.bold(`⌚ ${Logger.getTimestamp()}`);
	...
	static info(message: string): void {
		console.log(Logger.formatMessage(chalk.blue("INFO"), chalk.white(message)));
	}
	...
	static success(message: string): void {
		console.log(
			Logger.formatMessage(chalk.green("SUCCESS"), chalk.white(message)),
		);
	}
	...
	static warn(message: string): void {
		console.warn(
			Logger.formatMessage(chalk.yellow("WARN"), chalk.yellowBright(message)),
		);
	}
	...
	static error(message: string): void {
		console.error(
			Logger.formatMessage(chalk.red("ERROR"), chalk.redBright(message)),
		);
	}
	...
	static debug(message: string): void {
		console.debug(
			Logger.formatMessage(chalk.magenta("DEBUG"), chalk.gray(message)),
		);
	}
	...
	static command(
		...
		if (typeof messageOrOptions === "string") {
			...
			const message = `${chalk.gray("👤 User")}: ${chalk.yellow(username)} (${chalk.yellow(
				userId,
			...
			console.log(
				Logger.formatMessage(chalk.yellowBright("CMD"), chalk.white(message)),
			);
		}
	}
}
